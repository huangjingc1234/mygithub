...
static void MGC_HostSetDeviceAddress(void* pParam, uint16_t wTimer)
{
    ...
    MUSB_MemCopy(pEnumData->aSetupTx, MGC_aSetDeviceAddress, 
	sizeof(MGC_aSetDeviceAddress));
...
static void MGC_HostEnumerator(void* pParam, MUSB_ControlIrp* pIrp)
{
	...
	{
	    MGC_DIAG(1, pController, "Enumeration aborted");
...
#ifdef SET_ADDR_FIRST
	pServices->pfArmTimer(pServices->pPrivateData, 
		0, 10, FALSE, MGC_HostSetDeviceAddress);
#else
	pServices->pfArmTimer(pServices->pPrivateData, 
		0, 10, FALSE, MGC_HostGetShortDescriptor);
#endif
	return TRUE;
    }
    return FALSE;
