...
extern uint8_t MGC_aMsdData[4096+512];
#define MGC_DISK_ACAP sizeof(MGC_aMsdData)
#define MGC_DISK_END (MGC_aMsdData+MGC_DISK_ACAP)
...
#if 1
    if (!pPort->bIsHost) {
        pSource = (pSource+dwCount < MGC_DISK_END) ? pSource : (MGC_aMsdData+(pSource-MGC_aMsdData)%MGC_DISK_ACAP);
        dwCount = (pSource+dwCount > MGC_DISK_END) ? MGC_DISK_END-pSource : dwCount;
        dwCount32 = ((uint32_t)pSource & 3) ? 0 : (dwCount >> 2);
    }
#endif
...
#if 1
    if (!pPort->bIsHost) {
        pDest = (pDest+dwCount < MGC_DISK_END) ? pDest : (MGC_aMsdData+(pDest-MGC_aMsdData)%MGC_DISK_ACAP);
        dwCount = (pDest+dwCount > MGC_DISK_END) ? MGC_DISK_END-pDest : dwCount;
        dwCount32 = ((uint32_t)pDest & 3) ? 0 : (dwCount >> 2);
    }
#endif
    /* doublewords when possible */
    for(dwIndex = dwIndex32 = 0; dwIndex32 < dwCount32; dwIndex32++, dwIndex += 4)
