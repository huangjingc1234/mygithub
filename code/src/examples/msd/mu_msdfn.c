...
STATIC void MGC_MsdRxCbwComplete(void* pCompleteParam, MUSB_Irp* pIrp);
STATIC void MGC_MsdRxDataComplete(void* pCompleteParam, MUSB_Irp* pIrp);

STATIC void MGC_MsdTxDataComplete(void* pCompleteParam, MUSB_Irp* pIrp);
STATIC void MGC_MsdTxCswComplete(void* pCompleteParam, MUSB_Irp* pIrp);
...
//STATIC uint8_t MGC_aMsdData[1024*8192+512];
//static uint32_t MGC_dwMsdBlockCount = 1024 * 8192 / 512;
#define MGC_DISK_VCAP (1024*8192+512)
uint8_t MGC_aMsdData[4096+512];
#define MGC_DISK_ACAP sizeof(MGC_aMsdData)
uint32_t MGC_dwMsdBlockCount = MGC_DISK_VCAP / 512-1;
					   ...
					   MUSB_Pipe* ahPipe)
{
    ...
    MUSB_MemSet(MGC_aMsdData, 2, sizeof(MGC_aMsdData));
    dwStatus = MUSB_StartTransfer(&MGC_MsdRxCbwIrp);
