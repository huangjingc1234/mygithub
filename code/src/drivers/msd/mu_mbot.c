				       ...
				       MGC_MsdCmdSet* pCmdSet)
{
    ...
    MUSB_MemCopy(&(InEnd.UsbDescriptor), pInEnd, 
	sizeof(MUSB_EndpointDescriptor));
    MUSB_MemCopy(&(OutEnd.UsbDescriptor), pOutEnd, 
	sizeof(MUSB_EndpointDescriptor));
    ...
    pBot = (MGC_MsdBotProtocol*)MUSB_MemAlloc(
	sizeof(MGC_MsdBotProtocol));
    if(pBot)
    {
...
void MGC_BotControlIrpComplete(void* pParam, MUSB_ControlIrp* pIrp)
{
	...
	/* get lang IDs */
	pBot->bGetLang = TRUE;
	MUSB_MemCopy(pBot->aSetup, MGC_aMsdBotGetStringData,
	    sizeof(MGC_aMsdBotGetStringData));
